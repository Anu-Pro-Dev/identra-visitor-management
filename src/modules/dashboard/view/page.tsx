...
const leavesData = [
  { label: "Total Visitors", value: "05", icon: <FaUsers className="text-primary text-[1.3rem]" />},
  { label: "Total Meeting Hours", value: (<span>06:30<span className="text-xs text-muted-foreground font-normal">/hrs</span></span>),  icon: <IoAlarm className="text-primary text-[1.3rem]" /> },
  { label: "Expected Spending Time", value: <span>08:30<span className="text-xs text-muted-foreground font-normal">/hrs</span></span>, icon: <CgSandClock className="text-yellow-500 dark:text-yellow-400 text-[1.3rem]" /> },
  { label: "Actual Hours Spent", value: (<span>06:30<span className="text-xs text-muted-foreground font-normal">/hrs</span></span>), icon: <CgSandClock className="text-cyan-500 dark:text-cyan-400 text-[1.3rem]" /> },
  { label: "Upcoming Visitors", value: "02", icon:  <FaArrowUpRightFromSquare className="text-primary" /> },
  { label: "Pending Approvals", value: "01", icon: <FaArrowUpRightFromSquare className="text-primary" /> }
];
  ...
  return (
    <section className="flex flex-col gap-6 w-full">
      <header className="mb-2">
        ...
        <p className="text-muted-foreground text-sm mt-1">Welcome back! Here’s a quick overview of your visitor activity and meetings.</p>
      </header>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
        <LeaveCard leavesData={leavesData} />
        <VisitorList year={year} monthIndex={monthIndex} onMonthChange={handleMonthChange} months={months} />
      </div>
      <VisitorCalendar
        months={months}
